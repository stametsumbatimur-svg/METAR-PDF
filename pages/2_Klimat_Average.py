...
# --- FUNGSI VEKTORISASI FISIKA ---
def hitung_tekanan_uap_vec(suhu, rh):
    es = 6.112 * np.exp((17.67 * suhu) / (suhu + 243.5))
    res = (rh / 100.0) * es * 10
    return np.round(res, 2)

def hitung_dewpoint_vec(suhu, rh):
    a, b = 17.27, 237.7
    alpha = ((a * suhu) / (b + suhu)) + np.log(rh / 100.0)
    res = (b * alpha) / (a - alpha)
    return np.round(res, 2)

# --- FUNGSI EKSTRAKSI SANDI OPTIMAL ---
def ext_appp_fast(v, syn):
    if pd.isna(v): return np.nan
    if pd.notna(syn):
        exp = f"{int(round(abs(v) * 10)):03d}"
        tokens = re.findall(r'\b5\d{4}\b', str(syn).split('333')[0])
        for tk in tokens:
            if tk.endswith(exp): return int(tk[1:])
        for a in range(10):
            if f"5{a}{exp}" in str(syn).split('333')[0]: return int(f"{a}{exp}")
    return int(f"{'3' if v >= 0 else '8'}{int(round(abs(v) * 10)):03d}")

def ext_p24_fast(v, syn):
    if pd.isna(v): return np.nan
    if pd.notna(syn) and '333' in str(syn):
        exp = f"{int(round(abs(v) * 10)):03d}"
        tokens = re.findall(r'\b5[89]\d{3}\b', str(syn).split('333')[1])
        for tk in tokens:
            if tk.endswith(exp): return int(tk[2:]) if tk.startswith('58') else 500 + int(tk[2:])
        for prfx in ['58', '59']:
            if f"{prfx}{exp}" in str(syn).split('333')[1]: return int(exp) if prfx == '58' else 500 + int(exp)
...
if uploaded_file is not None:
    try:
        ...
        # Vektorisasi Perhitungan Fisika (Tanpa apply)
        if 'temp_drybulb_c_tttttt' in df_raw.columns and 'relative_humidity_pc' in df_raw.columns:
            df_raw['Tekanan_Uap_x10'] = hitung_tekanan_uap_vec(df_raw['temp_drybulb_c_tttttt'], df_raw['relative_humidity_pc'])
            df_raw['Dewpoint'] = hitung_dewpoint_vec(df_raw['temp_drybulb_c_tttttt'], df_raw['relative_humidity_pc'])
        ...
        with pd.ExcelWriter(buffer_matriks, engine='xlsxwriter') as writer:
            ...
            for kolom_csv, judul_param in parameter_mapping.items():
                if kolom_csv in df_bulan_ini.columns:
                    ...
                    # Ekstraksi Sandi 3 Jam & 24 Jam Cepat via List Comprehension & zip
                    if kolom_csv == 'pressure_3h_diff_mb_ppp' and 'encoded_synop' in df_bulan_ini.columns:
                        df_bulan_ini['sandi_5app'] = [ext_appp_fast(v, syn) for v, syn in zip(df_bulan_ini['pressure_3h_diff_mb_ppp'], df_bulan_ini['encoded_synop'])]
                        ...
                        df_bulan_ini['sandi_p24'] = [ext_p24_fast(v, syn) for v, syn in zip(df_bulan_ini['pressure_24h_diff_mb_p24'], df_bulan_ini['encoded_synop'])]
        ...
        # Ekstraksi Cepat Sandi ME45 via List Comprehension
        syn_col = df_merge['encoded_synop'] if 'encoded_synop' in df_merge.columns else [np.nan] * len(df_merge)
        df_out['appp'] = [ext_appp_fast(v, syn) for v, syn in zip(df_merge['pressure_3h_diff_mb_ppp'], syn_col)]
        df_out['P24P24P24'] = [ext_p24_fast(v, syn) for v, syn in zip(df_merge['pressure_24h_diff_mb_p24'], syn_col)]
        ...
        st.success("🎉 Berhasil! Seluruh pemrosesan data telah dioptimasi dengan vektorisasi.")
